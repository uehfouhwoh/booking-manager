...
class DatabaseService {
  ...
  static const List<String> defaultFacilities = [
    'Meeting Room',
    'Computer Lab',
    'Lecture Hall',
    'Projector Set',
  ...
  Stream<List<String>> getFacilities() {
    return _firestore.collection('settings').doc('campus').snapshots().map((
      ...
      if (snapshot.exists && snapshot.data()!.containsKey('facilities')) {
        return _cleanOptions(snapshot.data()!['facilities'], defaultFacilities);
      }
  ...
  Future<List<String>> getFacilitiesOnce() async {
    ...
    if (doc.exists && doc.data()!.containsKey('facilities')) {
      return _cleanOptions(doc.data()!['facilities'], defaultFacilities);
    }
    return defaultFacilities;
  }
  ...
  Future<void> addFacility(String facility) async {
    ...
    if (!doc.exists) {
      await docRef.set({
        'departments': defaultDepartments,
        'facilities': [...defaultFacilities, facility],
        'updatedAt': FieldValue.serverTimestamp(),
      });
    } else {
      await docRef.update({
